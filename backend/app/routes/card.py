...
@token_required
def get_cards(jwt_data, deck_id):
    user = db.session.get(User, jwt_data['user_id'])
    ...
    card_list = [card.get_json() for card in deck.cards]
...
def get_card(jwt_data, card_id):
    ...
    if not card.deck.shared and card.deck.owner_id != user.id:
        return jsonify({'message': 'You do not the deck to which this card pertains'}), 403
...
def update_card(jwt_data, card_id):
    ...
    data = request.json
    front = data.get('front') or card.front
    back = data.get('back') or card.back
