...
class TestAuthRoutes(TestEnvironment):
    ...
    def test_auth(self):
        login = {'username': 'Alfred', 'password': 'testpassword', 'tzutcdelta': 0}
        response = self.client.post('/auth', json=login)
        self.assertEqual(response.status_code, 200)

    def test_auth_invalid_tzutcdelta(self):
        login = {'username': 'Alfred', 'password': 'testpassword', 'tzutcdelta': 'invalid'}
        response = self.client.post('/auth', json=login)
    ...
    def test_auth_user_not_found(self):
        login = {'username': 'Alfred2', 'password': 'testpassword', 'tzutcdelta': 0}
        response = self.client.post('/auth', json=login)
        self.assertEqual(response.status_code, 404)

    def test_auth_invalid_password(self):
        login = {'username': 'Alfred', 'password': 'testpassword123', 'tzutcdelta': 0}
        response = self.client.post('/auth', json=login)
        self.assertEqual(response.status_code, 401)

    def test_auth_invalid_token(self):
        data = {'front': 'Front of Card', 'back': 'Back of Card', 'tzutcdelta': 0}
